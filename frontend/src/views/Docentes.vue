...
// emitir tickets (uno o los que falten) y ver QR. Calcado de Administrativos.
// Nota: aún no hay plantilla de ticket de docente, así que "ver" muestra el QR.
...
import { emitirTicketDocente, obtenerTicketQr } from '@/api/ticket.service'
import type {
  DocenteDetalleDto,
  DocenteDto,
...
const columnas: ColumnaTabla[] = [
  { clave: 'codigoDocente', titulo: 'Código docente', ancho: '150px' },
  { clave: 'nombreCompleto', titulo: 'Nombre completo' },
  { clave: 'ci', titulo: 'CI', ancho: '120px' },
  { clave: 'codigoTicket', titulo: 'Ticket', ancho: '140px' },
]

const sinTicket = computed(() => docentes.value.filter((d) => !d.idTicket))
const conTicket = computed(() => docentes.value.filter((d) => d.idTicket))
...
function formVacio(): DocenteDto {
  return { nombre: '', paterno: '', materno: '', ci: '', codigoDocente: '' }
}
...
async function eliminar(d: DocenteDetalleDto) {
  const ok = await confirmar({
    titulo: 'Eliminar docente',
    mensaje: `¿Eliminar a ${d.nombreCompleto}?`,
    textoConfirmar: 'Eliminar',
    peligro: true,
  })
  if (!ok) return
  try {
    await eliminarDocente(d.idDocente)
...
async function emitir(d: DocenteDetalleDto) {
  try {
    await emitirTicketDocente(d.idDocente)
...
async function verQr(d: DocenteDetalleDto) {
  if (!d.idTicket) return
  try {
    const blob = await obtenerTicketQr(d.idTicket)
    liberar()
    qrUrl.value = URL.createObjectURL(blob)
    codigoActual.value = d.codigoTicket || ''
...
<template>
  <div>
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <p class="ayuda">
        Columnas en este orden: <code>código docente, nombre completo, ci</code>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style="width:150px">Código docente</th>
    ...
    <ModalBase v-if="mostrarModal" titulo="Nuevo docente" @cerrar="mostrarModal = false">
      <form id="form-docente" @submit.prevent="guardar">
        <label>Código docente *</label><input v-model="form.codigoDocente" required />
    ...
    <ModalBase v-if="mostrarQr" :titulo="`Ticket ${codigoActual}`" @cerrar="cerrarQr">
      <div style="text-align:center">
        <p style="color:var(--texto-suave);font-size:13px;margin-top:0">
          (Aún sin plantilla de docente; se muestra el QR)
        </p>
