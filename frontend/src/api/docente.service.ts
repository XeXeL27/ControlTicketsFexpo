// Capa de API de Docentes (CRUD + importación CSV). Calcado de administrativo.
...
export function previsualizarDocentesCsv(archivo: File) {
  const fd = new FormData()
  fd.append('archivo', archivo)
  return http.post<PrevisualizacionDocCsvDto>('/docentes/previsualizar', fd).then((r) => r.data)
}
